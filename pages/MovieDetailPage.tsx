...
const MovieDetailPage: React.FC = () => {
  ...
  const getFilteredScreens = () => {
    return screens.map(screen => {
      // Find all groups for this screen on the selected date
      const relevantGroups = screen.showGroups.filter((group: any) => group.bookingDate === selectedDate);

      if (relevantGroups.length === 0) return { ...screen, showGroups: [] };

      // Merge showtimes from all groups for this date
      const allShowtimes = new Set<string>();
      let theatreName = '';

      relevantGroups.forEach((group: any) => {
        if (group.theatre) theatreName = group.theatre;
        if (Array.isArray(group.showtimes)) {
          group.showtimes.forEach((t: string) => allShowtimes.add(t));
        }
      });

      // Sort showtimes roughly by time
      const sortedShowtimes = Array.from(allShowtimes).sort((a, b) => {
      ...
      // Return screen with a SINGLE consolidated group
      return {
        ...screen,
        showGroups: [{
                  ...
                  <div className="flex space-x-1 sm:space-x-2 flex-wrap justify-center flex-1 overflow-x-auto">
                    {dateOptions.map((dateOption) => (
                      <button
                        ...
                        className={`px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap transition-all duration-300 ${dateOption.isSelected
                          ...
                          {group.showtimes.map((t: string) => {
                            ...
                            return (
                              <button
                                ...
                                className={`relative px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all duration-300 ${showtimeStatus.disabled
